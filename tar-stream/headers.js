...
/* csnf is NOT a tar file. just ignore magic numbers.
...
exports.encode = function (opts) {
  ...
  buf.write(name)
  // buf.write(encodeOct(opts.mode & MASK, 6), 100) // csnf
  buf.write(encodeOct(opts.uid, 6), 108)
  buf.write(encodeOct(opts.gid, 6), 116)
  buf.write(encodeOct(opts.size, 11), 124)
  // buf.write(encodeOct((opts.mtime.getTime() / 1000) | 0, 11), 136) // csnf

  buf[156] = ZERO_OFFSET + toTypeflag(opts.type)

  if (opts.linkname) buf.write(opts.linkname, 157)

  // USTAR_MAGIC.copy(buf, MAGIC_OFFSET) // csnf
  // USTAR_VER.copy(buf, VERSION_OFFSET) // csnf
...
exports.decode = function (buf, filenameEncoding) {
  ...
  const name = decodeStr(buf, 0, 100, filenameEncoding)
